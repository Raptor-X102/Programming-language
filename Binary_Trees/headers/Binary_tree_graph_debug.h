...
#define DOT_CALL "dot -Tpng "

#define PRINTOUT(...) fprintf(output_file, __VA_ARGS__)

const int Calloc_add = 20; // in case user inserts filename with 1 char extension. PNG extension requires 3 chars. Additionally 4 byte for SPACES, -o.

bool Tree_graph_debug(Node* root, const char* output_file_name, void (*Print_tree_data) (const void* data, FILE* file));
